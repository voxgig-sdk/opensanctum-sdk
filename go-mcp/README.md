...
[MCP](https://modelcontextprotocol.io) server exposing the Opensanctum SDK as
two agent tools — `opensanctum_list` and `opensanctum_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensanctum-mcp)
make build

# 2. Provide credentials via the environment
export OPENSANCTUM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user opensanctum \
  -- /absolute/path/to/opensanctum-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./opensanctum-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// opensanctum_list: first page of records
{ "entity": "place" }
{ "entity": "place", "query": { } }

// opensanctum_load: one record by id
{ "entity": "place", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensanctum-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user opensanctum \
     -- "$PWD"/dist/*/opensanctum-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `opensanctum_list` and `opensanctum_load` tools now appear
   in new sessions. Ask the agent to *"list place using opensanctum"*
   and it calls `opensanctum_list` with `{"entity":"place"}`.
...
```sh
export OPENSANCTUM_APIKEY=sk_live_xxx            # API key
export OPENSANCTUM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `opensanctum_list` tool
...
### Call the `opensanctum_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `opensanctum_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `opensanctum_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENSANCTUM_APIKEY` | API key sent with every request. |
| `OPENSANCTUM_BASE` | Optional override of the API base URL. |
...
```sh
./opensanctum-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"opensanctum_load","arguments":{"entity":"place","query":{"id":1}}}}'
```
