...
module OpensanctumOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if OpensanctumOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = OpensanctumOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| OpensanctumOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || OpensanctumOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = OpensanctumSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = OpensanctumError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { OpensanctumOmni.errify(err) }
