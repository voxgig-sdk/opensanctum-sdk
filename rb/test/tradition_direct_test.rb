...
class TraditionDirectTest < Minitest::Test
  def test_direct_list_tradition
    setup = tradition_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-tradition", setup[:live] ? "live" : "unit")
    if _should_skip
...
def tradition_direct_setup(mockres)
