...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = OpensanctumOmni.make_runner(TEST_JSON_FILE, OpensanctumSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    OpensanctumError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : OpensanctumOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = OpensanctumOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
