...
require_relative 'Opensanctum_types'
...
class OpensanctumSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpensanctumHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpensanctumHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpensanctumFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpensanctumUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpensanctumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpensanctumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpensanctumHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpensanctumHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpensanctumError.new(
        "graphql_error", "OpensanctumSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpensanctumSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
