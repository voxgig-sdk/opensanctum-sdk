...
class TraditionEntity
  ...
  def make
    opts = @_entopts.dup
    TraditionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpensanctumHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OpensanctumHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [TraditionListMatch, Hash, nil] match filter (any subset of
  #   Tradition fields); defaults to nil, treated as an empty match that lists all.
