...
// Opensanctum SDK Tradition entity
...
class TraditionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TraditionEntity($this->_client, $opts);
    }

    /**
     * @param Tradition|array $args Tradition data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpensanctumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Tradition|array The current Tradition data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Tradition fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpensanctumHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Tradition fields).
     */
     ...
     *
     * @param TraditionListMatch|array|null $reqmatch Match filter (any subset
     *   of Tradition fields) as an assoc-array; TraditionListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Tradition[]|array A list of Tradition items as assoc-arrays at
     *   the SDK boundary; throws OpensanctumError on failure (item-5 convention).
     */
