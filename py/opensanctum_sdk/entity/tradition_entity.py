...
from opensanctum_sdk.opensanctum_types import (
    Tradition,
    TraditionListMatch,
)
...
class TraditionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TraditionEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Tradition]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Tradition().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
