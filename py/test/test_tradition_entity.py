...
class TestTraditionEntity:

    def test_should_create_instance(self):
        testsdk = OpensanctumSDK.test(None, None)
        ent = testsdk.Tradition(None)
    ...
    def test_should_stream(self):
        ...
        base = OpensanctumSDK.test(seed, None)
        seen = list(base.Tradition(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from opensanctum_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = OpensanctumSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Tradition(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _tradition_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "tradition." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPENSANCTUM_TEST_TRADITION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        tradition_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.tradition")))
        tradition_ref01_data = None
        if len(tradition_ref01_data_raw) > 0:
...
def _tradition_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/tradition/TraditionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPENSANCTUM_TEST_TRADITION_ENTID"))
