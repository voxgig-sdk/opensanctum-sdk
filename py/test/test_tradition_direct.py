...
class TestTraditionDirect:

    def test_should_direct_list_tradition(self):
        setup = _tradition_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-tradition", "live" if setup["live"] else "unit")
        if _skip:
...
def _tradition_direct_setup(mockres):
