...
//     (\`new OpensanctumSDK(...)\` / \`OpensanctumSDK.test(...)\` become
//     \`OpensanctumSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"place":{"test01":{"id":"test01"}},"tradition":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpensanctumSDK(...)\` and any
// \`OpensanctumSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpensanctumSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpensanctumSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
