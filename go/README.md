...
## Install
```bash
go get github.com/voxgig-sdk/opensanctum-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/opensanctum-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/opensanctum-sdk/go=../opensanctum-sdk/go
```
...
func main() {
    ...
    // List place records — the value is the array of records itself.
    places, err := client.Place(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range places.([]any) {
        fmt.Println(item)
    }
}
```
...
place, err := client.Place(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(place) // the returned mock data
```
...
slice):
    ...
    place, err := client.Place(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // place is the returned record
...
```go
traditions, err := client.Tradition(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(traditions) // the array of records
```
...
// place.Data() now returns the place data from the last list
// place.Match() returns the last match criteria
```
