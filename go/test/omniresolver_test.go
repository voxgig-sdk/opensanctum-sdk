...
import (
	...
	omni "github.com/voxgig-sdk/opensanctum-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpensanctumError); is {
				out := map[string]any{
					"name":    "OpensanctumError",
...
func omniCtx(arg any, client *sdk.OpensanctumSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpensanctumSDK); is {
			client = live
