...
boru-driven command-line client **and** interactive REPL for the Opensanctum
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/opensanctum-cli)
make build

# 2. See usage (words, entities, env vars)
./opensanctum-cli --help

# 3. Provide credentials once, via the environment
export OPENSANCTUM_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./opensanctum-cli list place
./opensanctum-cli list tradition

# 5. Override the API base URL for a single call
OPENSANCTUM_BASE=https://api.example.com ./opensanctum-cli list place

# 6. No arguments -> interactive REPL
./opensanctum-cli
opensanctum> list place
opensanctum> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/opensanctum-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/opensanctum-cli list place
   ```
...
Run with no arguments to open a REPL (prompt `opensanctum>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `place`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/opensanctum-cli`. |
...
(`./opensanctum-cli <expr>`) and interactively (the REPL), and expressions compose the
