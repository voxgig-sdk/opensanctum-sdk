// Command Opensanctum-cli is a CLI + REPL for the Opensanctum SDK,
...
const prompt = "opensanctum"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "place tradition"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPENSANCTUM_APIKEY (api key), OPENSANCTUM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPENSANCTUM_APIKEY carries the API key and
	// OPENSANCTUM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENSANCTUM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENSANCTUM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpensanctumSDK(opts)
