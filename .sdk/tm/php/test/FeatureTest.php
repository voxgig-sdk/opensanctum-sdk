...
class FtCtrl extends OpensanctumControl
{
...
class FtClient
{
    ...
    private OpensanctumUtility $utility;

    public function __construct(OpensanctumUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpensanctumUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpensanctumContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpensanctumContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpensanctumContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpensanctumConfig::make_feature($name);
    ...
    private static function buildUrl(OpensanctumSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpensanctumError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpensanctumResponse($fetched);
        }
    ...
    private function fail(OpensanctumContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpensanctumContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
