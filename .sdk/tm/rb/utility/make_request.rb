...
module OpensanctumUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpensanctumResponse.new({})
    ...
    if fetch_err
      ...
      response = OpensanctumResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpensanctumResponse.new(fetched)
    else
