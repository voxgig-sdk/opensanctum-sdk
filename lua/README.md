...
The Lua SDK for the Opensanctum API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Place()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── opensanctum_sdk.lua    -- Main SDK module
...
The main module (`opensanctum_sdk`) exports the SDK constructor
