...
local TraditionEntity = {}
TraditionEntity.__index = TraditionEntity


function TraditionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TraditionEntity)
  self._name = "tradition"
  self._client = client
...
function TraditionEntity:get_name()
  return self._name
end


function TraditionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TraditionEntity.new(self._client, opts)
end
...
function TraditionEntity:mark_deleted()
  self._deleted = true
end


function TraditionEntity:deleted()
  return true == self._deleted
end


function TraditionEntity:data_set(args)
  if args ~= nil then
...
function TraditionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TraditionEntity:match_set(args)
  if args ~= nil then
...
function TraditionEntity:match_get()
...
function TraditionEntity:stream(action, args, callopts)
...
function TraditionEntity:_run_op(ctx, post_done)
  local utility = self._utility
